...
int Ypin = A1;  // declaring a variable and assigning it to one of the analog pin.
int Xval = 0;   //declaring a variable to store the value coming from the analog pin. 
int Yval = 0; //declaring a variable to store the value coming from the analog pin. 
...
void loop() {
  ...
  Yval = analogRead(Ypin);        // using the analogRead command to read the value of the Ypin; one of the analog pin used (value between 0 and 1023).
  Yval = map(Yval, 0, 1023, 0,180);   //using the map function to scale the value coming from that analog pin (value between 0 degree and 180 degree angles).
  myservo.write(Yval);       // using the myservo.write command to set the position of the servo motor based on the scaled value.
}
